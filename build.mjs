...
function metaDesc(p) {
  if (p.excerpt && p.excerpt.trim()) {
    const e = p.excerpt.replace(/\s+/g, " ").trim();
    return e.length > 200 ? e.slice(0, 197).replace(/\s+\S*$/, "") + "\u2026" : e;
  }
  // no excerpt -> derive from the first paragraph of the body
  const html = renderBody(p.body || "");
  const pm = html.match(/<p[^>]*>([\s\S]*?)<\/p>/i);
  const raw = pm ? pm[1] : html;
  const text = raw.replace(/<[^>]+>/g, " ").replace(/&[a-z#0-9]+;/gi, " ").replace(/\s+/g, " ").trim();
  if (!text) return (p.title || "").slice(0, 160);
...
function articleHtml(p, all) {
  ...
  const isExt = p.type === "external" && p.url;
  const main = isPdf
    ...
    : isExt
    ? `<div class="content"><p>This analysis was originally published on an external platform. Read the full piece at the source below.</p></div>
       <div class="pdf-actions"><a class="btn primary" href="${esc(p.url)}" target="_blank" rel="noopener">Read the full article \u2197</a></div>`
  ...
  const share = `<div class="share"><span>Share:</span>
    <button class="sbtn share-native" type="button" hidden data-title="${esc(p.title)}" data-url="${canonical}" aria-label="Share"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.6" y1="13.5" x2="15.4" y2="17.5"/><line x1="15.4" y1="6.5" x2="8.6" y2="10.5"/></svg> Share</button>
  ...
  const others = all.filter(x => x.id !== p.id && !x.draft)
    .sort((a, b) => (b.date || "").localeCompare(a.date || "")).slice(0, 3);
  const moreSec = others.length ? `<section class="section"><div class="wrap">
    <div class="sec-head"><div><span class="kicker">Reading</span><h2>More publications</h2></div><a class="more" href="/#/publications">All \u2192</a></div>
...
${FOOTER}
<script>var mb=document.getElementById("menuBtn");if(mb)mb.onclick=function(){document.getElementById("nav").classList.toggle("open");};
(function(){var c=window.matchMedia&&window.matchMedia("(pointer:coarse)").matches;var n=document.querySelector(".share-native");if(n&&navigator.share&&c){n.hidden=false;n.addEventListener("click",function(){navigator.share({title:n.dataset.title,url:n.dataset.url}).catch(function(){});});document.querySelectorAll(".share a.sbtn").forEach(function(a){a.style.display="none";});}})();</script>
</body>
</html>`;
}
...
(async () => {
  ...
  const printable = pubs.filter(p => p && p.id && !p.draft);
